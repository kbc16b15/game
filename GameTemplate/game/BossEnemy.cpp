...
void BossEnemy::Init(const char* modelName, D3DXVECTOR3	pos, D3DXQUATERNION	rot)
{
	m_sPos = pos;
	m_position = pos;
	m_rotation = rot;
	...
	//キャラクタコントローラを初期化。
	m_characterController.Init(8.0f, 2.5f, m_position);
	m_characterController.SetGravity(-10.0f);
...
void BossEnemy::Dead()
{
	...
	CubeCollision Cmath;
	if (Cmath.Cube(m_characterController.GetPosition(), g_game->GetPlayer()->Getpos(), 2.5f, 2.5f))
	{
		if (Cmath.Cube(m_characterController.GetPosition(), g_game->GetPlayer()->Getpos(), 2.0f, 2.0f))
		{
